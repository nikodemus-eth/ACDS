...
## 2026-03-16 — Test Suite Refactoring: Zero Mocks

- **Real HTTP servers beat fetch mocks for adapter testing.** `TestHttpServer` bound to port 0 catches bugs that `vi.stubGlobal('fetch')` cannot: header serialization, URL construction, query parameter encoding, connection lifecycle. The server is ~70 lines of code and replaces all fetch mocking across 5 adapter test files. The OS assigns the port, so tests never conflict. Start in `beforeAll`, close in `afterAll`, configure routes in `beforeEach`.
- **PGlite eliminates the "no database available" excuse for persistence tests.** `@electric-sql/pglite` provides a full PostgreSQL instance in-process via WASM — no Docker, no external services, no CI configuration. Tests run the same 8 migration SQL files that production uses. The only gotcha: PGlite's `query()` doesn't support multi-statement SQL (BEGIN/COMMIT blocks), so migration files need `exec()` (exposed as `execSQL()` on the wrapper).
- **In-memory repository implementations are not mocks.** `InMemoryProviderRepository` implements all 8 methods of `ProviderRepository` with real array filtering logic. This catches business logic bugs that a `vi.fn().mockResolvedValue([...])` cannot.
- **Console capture without vi.spyOn is trivial.** Save the original, replace with a collecting function, restore in `finally`. Used for fire-and-forget emit tests and error-logging tests.
- **Mock pool testing gives false confidence for SQL code.** PGlite tests verify actual behavior: write a row, read it back, assert values match. SQL can be wrong in many ways a string comparison wouldn't catch. *(This supersedes the lesson below from final stub elimination that recommended mock pools.)*
- **Coverage configuration must exclude type-only files.** A monorepo with strict TypeScript has hundreds of `.ts` files that export only `interface` and `type` declarations. Including them in coverage creates misleading percentages.
- **Lazy pg.Pool creation enables DI container testing without a database.** `pg.Pool` doesn't connect until the first query. `createDiContainer()` can be tested with a real pool without a running database.

## 2026-03-16 — Final Stub Elimination & Persistence Testing

- **Audit emitters should use fire-and-forget writes.** `PgApprovalAuditEmitter` and `PgRollbackAuditEmitter` use `.catch()` on the query promise instead of `await`. Audit logging should never block the critical path — if the DB write fails, log to stderr but don't crash the operation. This matches the pattern of the original `Noop` emitters (which used synchronous `console.info`) but now actually persists the data.
- **A single repository class can implement both reader and writer interfaces (CQRS-lite).** `PgAdaptationEventRepository` now implements both `AdaptationEventReader` (for the admin API) and `AdaptationLedgerWriter` (for the adaptation service). Same table, same class, but the interfaces presented to consumers are role-specific. The DI container exposes the same instance via two different container keys. This avoids class proliferation while maintaining interface segregation.
- **`ON CONFLICT DO UPDATE` provides idempotent upsert for credential storage.** `PgSecretCipherStore.store()` uses upsert semantics so calling `store()` twice for the same provider replaces the envelope rather than failing on the unique constraint. This is critical for secret rotation flows where the caller may not know whether a secret already exists.
- **Mock pool testing is the right approach for persistence unit tests.** Pg repository classes translate between domain types and SQL. The valuable test surface is the SQL generation and row mapping, not the database itself. Mocking `pool.query` lets us verify parameter binding, column mapping, and edge cases (null handling, JSON serialization) without needing a live database. Integration tests with a real database belong at a higher layer.
- **Migration schema should anticipate domain evolution.** The initial `adaptation_rollback_records` table had narrow columns (`snapshot_id`, `reason`, `executed_by`) while the domain type (`AdaptationRollbackRecord`) evolved to include full JSONB snapshots. Migration 008 adds the richer columns alongside the originals. Designing migration schemas to include JSONB columns for complex nested data from the start avoids this catch-up pattern.

## 2026-03-17 — Coverage Push to 99% and Systematic Bug Discovery

- **`.map(this.method)` is a silent JavaScript footgun.** Every persistence-pg repository had `result.rows.map(this.mapRow)` — which detaches the method from its object, losing `this`. The bug only manifests when the result has rows AND the mapRow function calls other instance methods (like `parseJsonArrayOrNull`). Single-row queries use `this.mapRow(row)` directly, which works fine. The fix is trivially simple (`(r) => this.mapRow(r)`) but the bug is invisible until you hit the exact codepath in production. Tests found it — 15 instances across 8 files.
- **Coverage percentages without file-level analysis are misleading.** At 79% coverage, the text-summary looked reasonable. But JSON coverage analysis revealed 59 individual files under 100%, many at single-digit coverage. Directory-level aggregates in the text reporter masked the true picture by averaging high-coverage files with low-coverage ones.
- **Exclude wiring code from coverage scope, not from testing.** Route files, DI containers, and app bootstrap are pure wiring — they connect pieces but contain no logic. Including them in coverage creates pressure to write tests that verify wiring connections (fragile, low-value) instead of business logic. Excluding them from the metric while still having integration tests that exercise the full stack is the right balance.
- **Module-level singletons break test isolation.** `getAppConfig()` caches its result in a module-level variable. Tests that depend on different config values can't get them because the first call wins. Route tests attempting to set env vars and register Fastify plugins failed because `authPreHandler` called `getAppConfig()` which returned a cached config from a different test. The fix is either DI injection or `resetModules()` — but the better fix is recognizing that route registration tests are low-value in the first place.
- **Parallel test-writing agents work when domains are independent.** Four agents each handling a distinct package/domain (persistence-pg, execution-orchestrator, policy-engine, API) produced 41 test files with 500+ new tests that all passed on first integration. The key is giving each agent a clear domain boundary with no overlapping files.

## 2026-03-17 — Process Swarm ↔ ACDS Cross-System Integration

- **Modify the seam, not the surface area.** The `ProbabilisticSynthesisAdapter` has 200+ lines of post-processing (symbolic token resolution, confidence labels, word count enforcement, platform neutrality checks). Instead of rewriting the entire adapter, we modified only the `_generate()` method — the single seam where LLM calls happen. All post-processing stays intact. This yielded ACDS routing with zero risk to existing pipeline logic.
- **Defense-in-depth fallback keeps systems independently operable.** When ACDS is configured, `_generate()` tries ACDS first, then falls back to direct Ollama. This means Process Swarm still works if ACDS is down, and ACDS still works without Process Swarm. Neither system depends on the other being available — the integration is additive, not mandatory.
- **Zero-dependency Python clients using stdlib are pragmatic.** The ACDS client uses only `urllib.request` and `json` — no `requests`, no `httpx`, no pip dependencies. This matches Process Swarm's existing pattern (the original Ollama calls used `urllib.request` too) and avoids dependency version conflicts across the monorepo.
- **Adapter registry is the extension point, not job definitions.** New tools slot into the registry → capability mapping → action type chain. Job definitions reference tools abstractly. This means adding XTTS and GRITS-ACDS was three code changes (adapter, `__init__.py`, `capability.py`) plus a test assertion update — the framework did the rest.

## 2026-03-17 — Per-Action Inference Override Design

1. **SQL NULL and UNIQUE constraints**: `UNIQUE(action_id, run_id)` doesn't prevent duplicate rows when `run_id` is NULL (NULL != NULL in SQL). Used check-then-update pattern instead of ON CONFLICT for upserts involving nullable columns.

2. **Config-as-routing-key pattern**: Adding `INFERENCE_PROVIDER` to the config dict lets adapters route without knowing about the override system. The adapter just reads a config key — it doesn't need to query the database or understand the override hierarchy. Clean separation of concerns.

3. **Override hierarchy matters**: Run-specific → action-level → system default. Two-tier fallback in `get_action_inference_override()` lets users set swarm-wide defaults that individual runs can override.

4. **Apple Intelligence bridge is stub-first**: The bridge compiles and runs on current macOS but uses stub responses until Foundation Models ships in macOS 26. Same API surface means zero code changes when real inference becomes available — just rebuild the bridge.

## 2026-03-17 — Production Deployment as launchd Services

- **launchd `KeepAlive` with crash-looping processes creates restart throttling.** When the ACDS API was missing `DATABASE_URL` and crashing immediately on startup, launchd's `KeepAlive` mechanism kept restarting it — but with increasing delays (throttling). After fixing the root cause, `launchctl stop/start` wasn't enough because the throttle state persisted. The fix was `launchctl unload` followed by `launchctl load` to fully reset the agent state. Always unload/reload after fixing a crash-loop rather than just stop/start.
- **`process.cwd()` in Node.js launchd agents is the plist's `WorkingDirectory`, not the project root.** The ACDS API's `loadJson()` and custom `.env` loader both resolve paths relative to `process.cwd()`. The launchd plist set `WorkingDirectory` to `apps/api/`, so `infra/config/profiles/modelProfiles.json` resolved to `apps/api/infra/config/...` which didn't exist. A symlink (`apps/api/infra → ../../infra`) was the cleanest fix — it preserves the plist's working directory for `.env` loading while making config paths resolve correctly. The alternative (changing `WorkingDirectory` to the repo root) would have broken the `.env` loader.
- **Custom `.env` loaders that use `process.cwd()` need their file co-located with the launch point.** The ACDS API's `loadDotEnvFile()` reads from `path.resolve(process.cwd(), '.env')`. When launched via launchd with `WorkingDirectory: apps/api`, the `.env` must be at `apps/api/.env` — not the repo root. This is obvious in hindsight but easy to miss when the development workflow uses `pnpm dev` from the monorepo root.
- **PostgreSQL must be installed before the API can start — dependency ordering matters for launchd.** launchd doesn't support inter-agent dependency ordering like systemd's `After=`. All agents start simultaneously at login. The ACDS API connects to PostgreSQL lazily (first query, not at pool creation), so this works in practice — but if PostgreSQL is slow to start, the first few API requests would fail. A health-check retry loop in the API startup would be more robust.
- **Sensitive files (`.env`, `master.key`) must not be committed.** The `.env` file contains the database password and session secret. The master key file contains the AES-256-GCM encryption key. Both are in `.gitignore`. When deploying to a new machine, these must be provisioned separately — they are not in the repository.
- **Symlinks in working directories are invisible to the application.** `path.resolve(process.cwd(), 'infra/config/...')` follows symlinks transparently. The API has no idea that `apps/api/infra` points to `../../infra`. This is a strength — the fix required zero code changes — but it means `ls` in the working directory can be misleading. Document symlinks in deployment topology.

## 2026-03-20 — Apple Framework Availability vs Headless Usage

1. **Apple's `ImageCreator` requires visible foreground context — no workaround exists.** Tested: launchd agent, `.app` bundle via `open`, `NSApplication.shared.run()` with `.accessory` activation policy, `LSUIElement: true` + `LSBackgroundOnly: false`. All throw `backgroundCreationForbidden`. Apple intentionally blocks headless/background image generation. Unlike every other Apple framework we tested (`SFSpeechRecognizer`, `AVSpeechSynthesizer`, `Vision`, `SoundAnalysis`, `NLLanguageRecognizer`, `Translation`, `FoundationModels`), `ImageCreator` is the only one that refuses to run without a visible window. Don't build infrastructure around APIs before confirming they work headlessly.

2. **macOS 26 `TranslationSession` works headlessly but requires pre-installed language packs.** The new `init(installedSource:target:)` API lets you create a `TranslationSession` directly without SwiftUI — a major improvement over the SwiftUI-only `.translationTask` modifier in earlier OS versions. However, it only works with already-downloaded language packs. There's no programmatic download API — language packs require System Settings > General > Language & Region > Translation Languages. Always probe with `LanguageAvailability.status()` before attempting translation.

3. **Swift 6 strict concurrency requires `@Sendable` Task closures and thread-safe boxes.** The `DispatchSemaphore` + `Task` pattern for bridging async Apple APIs to sync NIO handlers breaks under Swift 6's strict concurrency checking if the captured variables aren't `Sendable`. Solution: capture values into `let` constants before the closure, use `@Sendable in` annotation, and use a thread-safe `ResultBox<T>` with `NSLock` for returning results.
...
7. **`ExplorationPolicy` dual-caller semantics require explicit config detection.** `computeExplorationRate(familyState, config?)` serves two callers: `shouldExplore(familyState)` with no config should use the stored `familyState.explorationRate`, while direct calls with explicit config should use config-based calculation. Using `Object.keys(config).length > 0` distinguishes these callers better than checking individual config properties, because partial configs (e.g., `{ consequenceLevel: 'low' }` without `baseRate`) should still trigger config-based calculation.

8. **Stale compiled `.js` files in TypeScript `src/` directories cause vitest to load outdated code.** When `tsc` compiles `.ts` to `.js` in the same directory (non-standard for a monorepo), vitest's module resolution may pick up the `.js` file instead of the `.ts` source. This causes "not a function" errors for methods that clearly exist in the `.ts` source. The fix is to delete all `.js`/`.d.ts` files from `packages/*/src/` and ensure `tsc` outputs to `dist/` instead.

9. **Frontend `fetch()` without auth headers fails behind authenticated API routes.** The `InputRenderer` was calling `/api/providers/translation/languages` with raw `fetch()`, missing the `x-admin-session` auth header. The fix is to use the shared `apiClient` wrapper that includes auth automatically. Every API call from the frontend should go through `apiClient`, never raw `fetch()`.
