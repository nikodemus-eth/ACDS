...
describe('Integration: OCR Execution Path', () => {
  ...
  it('extract text from screenshot returns structured extraction', async () => {
    const response = await orchestrator.executeTask('extract text from this screenshot', {
      input: { imageData: 'base64screenshotdata...' },
    });

    expect(response.metadata.methodId).toBe('apple.vision.ocr');

    const output = response.output as { extractedText: string; confidence: number; regions: unknown[] };
