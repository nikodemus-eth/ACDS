...
speaks generic *action dicts*) and the OpenShell pipeline (which speaks
...
class Normalizer:
    ...
    def normalize(
        ...
        CommandEnvelope | None
            ...
            registered as an OpenShell command (passthrough signal).
        """
        tool_name = action.get("tool_name", "")
        spec = self.registry.get_spec(tool_name)
        if spec is None:
            return None  # Not an OpenShell command -- pass through
