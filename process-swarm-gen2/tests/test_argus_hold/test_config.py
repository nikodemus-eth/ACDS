"""Tests for swarm.argus_hold.config — ARGUSHoldConfig."""
...
class TestARGUSHoldConfig:
    """Tests for ARGUSHoldConfig.for_run() and field defaults."""

    def test_for_run_creates_config(self, tmp_path):
        cfg = ARGUSHoldConfig.for_run(tmp_path, "run-42")
        assert isinstance(cfg, ARGUSHoldConfig)
    ...
    def test_artifact_and_ledger_roots(self, tmp_path):
        cfg = ARGUSHoldConfig.for_run(tmp_path, "run-42")
        assert "argus_hold" in cfg.artifact_root
        assert "argus_hold" in cfg.ledger_root
