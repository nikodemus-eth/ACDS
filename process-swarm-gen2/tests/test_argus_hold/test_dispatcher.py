"""Tests for swarm.argus_hold.dispatcher — ARGUSHoldDispatcher (full pipeline)."""
...
from swarm.argus_hold.models import CommandResult, StageVerdict
...
def _make_dispatcher(tmp_path, allowed_hosts=None):
    ...
    return ARGUSHoldDispatcher(cfg), ws


class TestDispatcherHandles:
    """Tests for ARGUSHoldDispatcher.handles()."""
...
class TestDispatcherLedgerIntegration:
    ...
    def test_successful_command_has_ledger(self, tmp_path):
        ...
        ledger_path = Path(disp.config.ledger_root) / "argus_hold_ledger.jsonl"
...
class TestDispatcherToToolResult:
    ...
    def test_converts_success(self, tmp_path):
        ...
        tool_result = ARGUSHoldDispatcher.to_tool_result(cmd_result)
        assert isinstance(tool_result, ToolResult)
        assert tool_result.success is True
        assert tool_result.metadata["argus_hold"] is True
...
class TestDispatcherNoAdapter:
    ...
    def test_no_adapter_for_namespace_returns_failure(self, tmp_path):
        ...
        real_specs = Path(__file__).resolve().parent.parent.parent / "swarm" / "argus_hold" / "command_specs"
