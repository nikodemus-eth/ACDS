...
## 2026-03-18 — ACDS Sovereign Runtime

- **Build in taxonomy order, not feature order.** The 4-pass sequence (domain → pipeline → adapters → observability) prevented architectural drift. Each pass built on verified foundations. If you skip ahead to adapters before enforcing the taxonomy model, you recreate the ambiguity you designed out.
- **Discriminated unions enforce class separation at compile time.** TypeScript's `type: "provider" | "capability" | "session"` discriminant means cross-class operations fail type-checking, not just runtime checks. Three enforcement layers (types, registry, policy) for the same rule.
- **Platform boundary fakes are not mocks.** The Apple method fakes implement real interfaces with deterministic output. The architecture is real; only the OS call is fake. This is the difference between "faking the system" and "faking the platform."
- **Double enforcement of cross-class fallback.** Both the execution planner AND the policy engine independently block cross-class fallback. Removing one check still leaves the other. Defense in depth matters for governance rules.
- **1000-run determinism tests prove determinism; single-run tests do not.** Running the same request once proves it works. Running it 1000 times proves it's deterministic. The routing path must be identical on every run, not just usually identical.
- **100% coverage is achievable without mocks when you separate concerns correctly.** 44 source files, 368 tests, 100% on all four metrics. The key: pure functions for resolver/planner logic, interfaces for platform boundaries, typed errors for every failure path.
- **Red team tests should think like an attacker, not a developer.** Taxonomy boundary attacks, silent escalation, cross-class injection, input injection, telemetry poisoning — these are adversarial scenarios, not edge cases. They test whether the system's governance is real or performative.
